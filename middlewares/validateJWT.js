...
const authMiddleware = (req, res, next) => {
  ...
  if (isEmpty(userID)) return res.status(401).json({ message: "User id is missing in the header." });
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    if (userID != userId) return res.status(401).json({ message: "This token is for different user." });
